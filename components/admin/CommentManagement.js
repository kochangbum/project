...
const CommentManagement = () => {
  ...
  useEffect(() => {
    const fetchCommentsAndViews = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3000/community/post/comments`,
          {
            withCredentials: true, // 로그인 정보 포함
          }
        );

        const commentData = response.data.comments; // 댓글 데이터
        const viewData = response.data.views; // 조회수 데이터

        setComments(commentData); // 댓글 데이터 설정
        setPostViews(viewData); // 댓글의 조회수 데이터 설정
      } catch (error) {
        console.error("Error fetching comments and views:", error);
      }
    };

    fetchCommentsAndViews(); // 데이터 호출
  }, []);
  ...
  const tableData = paginatedData.map((item, index) => ({
    ...
    date: item.date,
    viewCount: item.postView || 0, // 조회수 추가 (없을 경우 0으로 표시)
    manage: (
      <span
        style={{ cursor: "pointer" }}
        onClick={() =>
              ...
              <div className="modal-content">
                ...
                <div className="modal-header">
                  <div className="modal-header-left">
                    ...
                    />
                    <p>{selectedComment.categoryName}</p>
                  </div>
                  <div className="modal-header-right">
                    <p>
                      <strong></strong> {formatDate(selectedComment.date)}
                    </p>
                  </div>
                </div>
